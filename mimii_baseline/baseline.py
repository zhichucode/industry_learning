...
def file_load(wav_name, mono=False):
    ...
    excel_data = pd.read_excel(wav_name)
    excel_data = excel_data.dropna(axis=0, how='any')
    y_list = list(excel_data['1号球磨轴承振动_Y'])[::-1]
    fs = 500
    out_f = 'out.wav'
    wavf.write(out_f, fs, numpy.array(y_list))

    try:
        return librosa.load(out_f, sr=None, mono=mono)
    except:
...
def demux_wav(wav_name, channel=0):
    ...
    """
    try:
        logger.info("demux_wav -> {}".format(wav_name))
...
def file_to_vector_array(file_name,
    ...
    mel_spectrogram = librosa.feature.melspectrogram(y=y,
                                                     ...
                                                     n_mels=n_mels,
                                                     power=power)
    logger.info("mel_spectrogram.shape : {}".format(mel_spectrogram.shape))
    ...
    logger.info("log_mel_spectrogram.shape : {}".format(
        log_mel_spectrogram.shape))
    logger.info("vectorarray_size : {}".format(vectorarray_size))
    ...
    for t in range(frames):
        vectorarray[:, n_mels * t: n_mels *
                    (t + 1)] = log_mel_spectrogram[:, t: t + vectorarray_size].T
...
def list_to_vector_array(file_list,
    ...
    for idx in tqdm(range(len(file_list)), desc=msg):
        logger.info(f"{msg} : {file_list[idx]}")
        logger.error("Errors")
        ...
        vector_array = file_to_vector_array(file_list[idx],
                                            ...
                                            hop_length=hop_length,
                                            power=power)
        logger.info("vector_array.shape : {}".format(vector_array.shape))
        logger.info("idx : {}".format(idx))
        logger.info("vector_array.shape[0] : {}".format(vector_array.shape[0]))
        logger.info("len(file_list) : {}".format(len(file_list)))
...
def dataset_generator(target_dir,
    ...
    normal_files = sorted(glob.glob(
        os.path.abspath("{dir}/{normal_dir_name}/*.{ext}".format(dir=target_dir,
                                                                 ...
                                                                 ext=ext))))
    logger.info("{dir}/{normal_dir_name}/*.{ext}".format(dir=target_dir,
                                                         normal_dir_name=normal_dir_name,
                                                         ext=ext))
    logger.info("normal_files : {}".format(normal_files))
...
def keras_model(inputDim):
    ...
    x = Dense(128, activation='relu')(x)
    x = Dense(128, activation='relu')(x)
    x = Dense(inputDim, activation=None)(x)
...
if __name__ == "__main__":
    ...
    dirs = sorted(glob.glob(os.path.abspath(
        "{base}/*/*".format(base=param["base_directory"]))))
    logger.info("dirs : {}".format(dirs))

    # setup the result
    result_file = "{result}/{file_name}".format(
    ...
    for dir_idx, target_dir in enumerate(dirs):
        ...
        model = keras_model(param["feature"]["n_mels"]
        ...
        for num, file_name in tqdm(enumerate(eval_files), total=len(eval_files)):
            logger.info("eval_file : {}".format(file_name))
            try:
                data = file_to_vector_array(file_name,
                                            ...
                                            hop_length=param["feature"]["hop_length"],
                                            power=param["feature"]["power"])
                error = numpy.mean(numpy.square(
                    data - model.predict(data)), axis=1)
